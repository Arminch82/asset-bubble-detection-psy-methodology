...
prices <- tq_get(c("MSFT", "^GSPC", "^IXIC"), get = "stock.prices",
...
prices_wide <- prices|>
  ...
  rename(msft = MSFT, sp500 = `^GSPC`, nasdaq = `^IXIC`) |>
  drop_na()

prices_clean <- prices_wide

# Revenue
revenue <- read.csv("msft_revenue.csv") |>
...
period_1 <- prices_wide |>
  ...
  mutate(
    msft_norm = (msft / first(msft)) * 100,
    sp500_norm = (sp500 / first(sp500)) * 100,
    price_ratio = msft_norm / sp500_norm,
    sales = sales / 1000,
    price_sales = msft / sales) 
...
msft_m1_p1
ggsave("msft_price_sales_p1.pdf", plot = msft_m1_p1, width = 6, height = 4)
...
monthly_ff_3_factors <- monthly_ff_3_factors |>
  ...
  mutate(
    ...
    M_hat = first(log(msft)) + cumsum(fitted_delta_p_adj),
    IC = log(msft) - M_hat
  )
...
msft_ic_p1
ggsave("msft_ic_p1.pdf", plot = msft_ic_p1, width = 6, height = 4)
...
monthly_ff_5_factors <- monthly_ff_5_factors |>
  arrange(date) |>
  drop_na(msft, nasdaq, delta_p, delta_n, RF_log, SMB_dec, HML_dec, excess_return, 
          RMW_dec, CMA_dec)
...
msft_ic2_p1
ggsave("msft_ic2_p1.pdf", plot = msft_ic2_p1, width = 6, height = 4)
...
msft_m2_p1
ggsave("msft_price_ratio_p1.pdf", plot = msft_m2_p1, width = 6, height = 4)
