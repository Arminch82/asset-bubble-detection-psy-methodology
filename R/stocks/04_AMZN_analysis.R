...
prices <- tq_get(c("AMZN", "^GSPC", "^IXIC"), get = "stock.prices",
...
prices_wide <- prices|>
  ...
  rename(amzn = AMZN, sp500 = `^GSPC`, nasdaq = `^IXIC`) |>
  drop_na()

prices_clean <- prices_wide

# Revenue
revenue <- read.csv("amzn_revenue.csv") |>
...
period_1 <- prices_wide |>
  ...
  mutate(
    amzn_norm = (amzn / first(amzn)) * 100,
    sp500_norm = (sp500 / first(sp500)) * 100,
    price_ratio = amzn_norm / sp500_norm,
    sales = sales / 1000,
    price_sales = amzn / sales) 
...
amzn_m1_p1
ggsave("amzn_price_sales_p1.pdf", plot = amzn_m1_p1, width = 6, height = 4)
...
monthly_ff_3_factors <- monthly_ff_3_factors |>
  arrange(date) |>
  drop_na(amzn, nasdaq, delta_p, delta_n, RF_log, SMB_dec, HML_dec, excess_return)
...
monthly_ff_3_factors <- monthly_ff_3_factors |>
  ...
  mutate(
    ...
    M_hat = first(log(amzn)) + cumsum(fitted_delta_p_adj),
    IC = log(amzn) - M_hat
  )
...
amzn_ic_p1
ggsave("amzn_ic_p1.pdf", plot = amzn_ic_p1, width = 6, height = 4)
...
amzn_ic2_p1
ggsave("amzn_ic2_p1.pdf", plot = amzn_ic2_p1, width = 6, height = 4)
...
amzn_m2_p1
ggsave("amzn_price_ratio_p1.pdf", plot = amzn_m2_p1, width = 6, height = 4)
