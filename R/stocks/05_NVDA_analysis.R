...
prices <- tq_get(c("NVDA", "^GSPC", "^IXIC", "^VIX"), get = "stock.prices",
...
prices_wide <- prices|>
  ...
  rename(nvda = NVDA, sp500 = `^GSPC`, nasdaq = `^IXIC`, vix = "^VIX") |>
  drop_na()

prices_clean <- prices_wide

# Revenue
revenue <- read.csv("nvda_revenue.csv") |>
  mutate(date = as.Date(date))

sp_revenue <- read.csv("sp500_sales.csv") |>
  rename(date = Date)|>
  mutate(date = as.Date(date))

prices_wide <- prices_wide |>
  left_join(revenue, by = "date") |>
  left_join(sp_revenue, by = "date") |>
  arrange(date) |>
...
period_1 <- prices_wide |>
  ...
  mutate(
    nvda_norm = (nvda / first(nvda)) * 100,
    sp500_norm = (sp500 / first(sp500)) * 100,
    price_ratio = nvda_norm / sp500_norm,
    sales = sales / 1000,
    price_sales = nvda / sales,
    sp500_price_sales = sp500 / Value) 
...
# Calculate Severity for Price/Sales Period 1
...
nvda_m1_p1
ggsave("nvda_price_sales_p1.pdf", plot = nvda_m1_p1, width = 6, height = 4)

# PSY test on period 2
...
ff_3_reg <- lm(delta_p ~ 0 + excess_return + SMB_dec + HML_dec, data = training_data)
...
monthly_ff_3_factors <- monthly_ff_3_factors |>
  ...
  mutate(
    ...
    M_hat = first(log(nvda)) + cumsum(fitted_delta_p_adj),
    IC = log(nvda) - M_hat
  )
monthly_ff_3_factors_p1 <- monthly_ff_3_factors |>
  filter(date <= as.Date("2022-11-01"))
monthly_ff_3_factors_p2 <- monthly_ff_3_factors |> filter(date >= as.Date("2022-12-01") & date <= as.Date("2025-01-31"))
...
nvda_ic_p1
ggsave("nvda_ic_p1.pdf", plot = nvda_ic_p1, width = 6, height = 4)

# Period 2
y_p2_ff3 <- as.numeric(monthly_ff_3_factors_p2$IC)

radf_result <- radf(y_p2_ff3, minw = 24, lag = 1)
cv_gsadf    <- radf_wb_cv2(y_p2_ff3, nboot = 2000, minw = 24)
summary(radf_result, cv_gsadf)

min_window <- 24
max_lag    <- 4
ic_method  <- 2
bsadf_result <- PSY(y = y_p2_ff3, 
                   swindow0 = min_window, 
                   IC = ic_method, 
                   adflag = max_lag)
cv_result <- cvPSYwmboot(y = y_p2_ff3, 
                         swindow0 = min_window, 
                         IC = ic_method, 
                         adflag = max_lag, 
                         Tb = length(y_p2_ff3), 
                         nboot = 2000)
gsadf_stat <- max(bsadf_result, na.rm = TRUE)
list(
  GSADF_Statistic = gsadf_stat,
  Critical_Values = cv_result
)
...
monthly_ff_5_factors <- monthly_ff_5_factors |>
  arrange(date) |>
  drop_na(nvda, nasdaq, delta_p, delta_n, RF_log, SMB_dec, HML_dec, excess_return, 
          RMW_dec, CMA_dec)
...
nvda_ic2_p1
ggsave("nvda_ic2_p1.pdf", plot = nvda_ic2_p1, width = 6, height = 4)
...
nvda_m2_p1
ggsave("nvda_price_ratio_p1.pdf", plot = nvda_m2_p1, width = 6, height = 4)
