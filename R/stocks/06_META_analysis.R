...
prices <- tq_get(c("META", "^GSPC", "^IXIC"), get = "stock.prices",
...
prices_wide <- prices|>
  ...
  rename(meta = META, sp500 = `^GSPC`, nasdaq = `^IXIC`) |>
  drop_na()

prices_clean <- prices_wide

# Revenue
revenue <- read.csv("meta_revenue.csv") |>
...
period_1 <- prices_wide |>
  ...
  mutate(
    meta_norm = (meta / first(meta)) * 100,
    sp500_norm = (sp500 / first(sp500)) * 100,
    price_ratio = meta_norm / sp500_norm,
    sales = sales / 1000,
    price_sales = meta / sales) 
...
meta_m1_p1
ggsave("meta_price_sales_p1.pdf", plot = meta_m1_p1, width = 6, height = 4)
...
monthly_ff_3_factors <- monthly_ff_3_factors |>
  arrange(date) |>
  drop_na(meta, nasdaq, delta_p, delta_n, RF_log, SMB_dec, HML_dec, excess_return)
...
monthly_ff_3_factors <- monthly_ff_3_factors |>
  ...
  mutate(
    ...
    M_hat = first(log(meta)) + cumsum(fitted_delta_p_adj),
    IC = log(meta) - M_hat
  )
...
meta_ic_p1
ggsave("meta_ic_p1.pdf", plot = meta_ic_p1, width = 6, height = 4)
...
meta_ic2_p1
ggsave("meta_ic2_p1.pdf", plot = meta_ic2_p1, width = 6, height = 4)
...
meta_m2_p1
ggsave("meta_price_ratio_p1.pdf", plot = meta_m2_p1, width = 6, height = 4)
