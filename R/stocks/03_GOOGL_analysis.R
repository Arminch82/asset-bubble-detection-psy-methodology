...
prices <- tq_get(c("GOOG", "^GSPC", "^IXIC"), get = "stock.prices",
...
prices_wide <- prices|>
  ...
  rename(goog = GOOG, sp500 = `^GSPC`, nasdaq = `^IXIC`) |>
  drop_na()

prices_clean <- prices_wide

# Revenue
revenue <- read.csv("goog_revenue.csv") |>
...
period_1 <- prices_wide |>
  ...
  mutate(
    goog_norm = (goog / first(goog)) * 100,
    sp500_norm = (sp500 / first(sp500)) * 100,
    price_ratio = goog_norm / sp500_norm,
    sales = sales / 1000,
    price_sales = goog / sales) 
...
goog_m1_p1
ggsave("goog_price_sales_p1.pdf", plot = goog_m1_p1, width = 6, height = 4)

# Period 2
y_p2_ps <- as.numeric(period_2$price_sales)

radf_result <- radf(y_p2_ps, minw = 24)
cv_gsadf    <- radf_wb_cv(y_p2_ps, nboot = 2000, minw = 24)
...
monthly_ff_3_factors <- monthly_ff_3_factors |>
  arrange(date) |>
  drop_na(goog, nasdaq, delta_p, delta_n, RF_log, SMB_dec, HML_dec, excess_return)
...
monthly_ff_3_factors <- monthly_ff_3_factors |>
  ...
  mutate(
    ...
    M_hat = first(log(goog)) + cumsum(fitted_delta_p_adj),
    IC = log(goog) - M_hat
  )
...
goog_ic_p1
ggsave("goog_ic_p1.pdf", plot = goog_ic_p1, width = 6, height = 4)

# Period 2

y_p2_ff3 <- as.numeric(monthly_ff_3_factors_p2$IC)

radf_result <- radf(y_p2_ff3, minw = 24)
cv_gsadf    <- radf_wb_cv(y_p2_ff3, nboot = 2000, minw = 24)
summary(radf_result, cv_gsadf)
...
goog_ic2_p1
ggsave("goog_ic2_p1.pdf", plot = goog_ic2_p1, width = 6, height = 4)
...
goog_m2_p1
ggsave("goog_price_ratio_p1.pdf", plot = goog_m2_p1, width = 6, height = 4)
