...
# Data collection
prices <- tq_get(c("TSLA", "^GSPC", "^IXIC", "^VIX"), get = "stock.prices",
...
prices_wide <- prices|>
  ...
  rename(tsla = TSLA, sp500 = `^GSPC`, nasdaq = `^IXIC`, vix = "^VIX") |>
  drop_na()

prices_clean <- prices_wide

# Revenue
revenue <- read.csv("tsla_revenue.csv") |>
...
period_1 <- prices_wide |>
  ...
  mutate(
    tsla_norm = (tsla / first(tsla)) * 100,
    sp500_norm = (sp500 / first(sp500)) * 100,
    price_ratio = tsla_norm / sp500_norm,
    sales = sales / 1000,
    price_sales = tsla / sales,
...
tsla_m1_p1
ggsave("tsla_price_sales_p1.pdf", plot = tsla_m1_p1, width = 6, height = 4)
...
monthly_ff_3_factors <- monthly_ff_3_factors |>
  arrange(date) |>
  drop_na(tsla, nasdaq, delta_p, delta_n, RF_log, SMB_dec, HML_dec, excess_return)
...
monthly_ff_3_factors <- monthly_ff_3_factors |>
  ...
  mutate(
    ...
    M_hat = first(log(tsla)) + cumsum(fitted_delta_p_adj),
    IC = log(tsla) - M_hat
  )
...
tsla_ic_p1
ggsave("tsla_ic_p1.pdf", plot = tsla_ic_p1, width = 6, height = 4)
...
tsla_ic2_p1
ggsave("tsla_ic2_p1.pdf", plot = tsla_ic2_p1, width = 6, height = 4)
...
tsla_m2_p1
ggsave("tsla_price_ratio_p1.pdf", plot = tsla_m2_p1, width = 6, height = 4)
